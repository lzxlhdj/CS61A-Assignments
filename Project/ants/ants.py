...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container == True and place.ant.can_contain(self): #如果原本存在的是容器，能存自己, 就存
                place.ant.store_ant(self)
            elif self.is_container == True and self.can_contain(place.ant): #如果自己能包含原有的蚂蚁
...
class ThrowerAnt(Ant):
    ...
    lower_bound = 0
    upper_bound = float('inf') #这两个是投掷蚂蚁能投掷到的范围的上界和下界（闭区间），一般的为0和无穷大，子类可能不是
    food_cost = 3
    ...
    def nearest_bee(self):
        ...
        while steps <= self.upper_bound and not target.is_hive: #不能超过上界 也不能是蜂巢
            if target.bees != [] and steps >= self.lower_bound: #找到大于下界的 有蜜蜂的位置就ok
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        origin_bees = self.place.bees[:]
        super().reduce_health(amount)
        if self.health <= 0:
            reflected_damage += self.damage
        for bee in origin_bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        assert self.cooldown >= 0, "冷却不可以小于0"

        if self.cooldown == 0:
            if self.place.bees: #自己这个位置有蜜蜂 就随机挑一个吃
                bee = random_bee(self.place.bees)
                bee.reduce_health(bee.health) #吃掉
                self.cooldown = HungryAnt.chew_cooldown #重置冷却
        else :
            self.cooldown -= 1
# END Problem 7
...
# BEGIN Problem 9
# The TankAnt class
class TankAnt(ContainerAnt): #是一个容器，同时可以每回合对所在位置所有蜜蜂造成一点伤害
    name = 'Tank'
    food_cost = 6
    implemented = True
    damage = 1
    ...
    def action(self, gamestate):
        ...
        origin_bees = self.place.bees[:] #每次群体伤害必须遍历的是副本！！！！
        for bee in origin_bees:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while behind_place != None: #如果是None就断开循环
            ant = behind_place.ant
            if ant != None: #当这里有蚂蚁,double
                ant.double()
                if ant.is_container == True and ant.ant_contained != None : #如果是容器且包含了蚂蚁，里面的也要double
